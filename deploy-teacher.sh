...
echo -e "${BLUE}====================================================${NC}"
echo -e "${BLUE}    👨‍🏫 OpenGoo! Secure Teacher Hub Deployer        ${NC}"
echo -e "${BLUE}====================================================${NC}"
...
GCP_PROJECT_ID=""
GCP_SERVICE_NAME="opengoo-teacher"
GCP_REGION=""
...
echo -e "\n${YELLOW}Submitting build to Google Cloud Build (Teacher app)...${NC}"
echo -e "Image tag: ${BLUE}${IMAGE_TAG}${NC}"

gcloud builds submit --config=cloudbuild-teacher.yaml --substitutions="${SUB_LIST}" .

echo -e "${GREEN}✓ Cloud Build finished successfully!${NC}"

# 7. Deploy to Google Cloud Run with IAP Enabled
echo -e "\n${YELLOW}Deploying container to Google Cloud Run...${NC}"
echo -e "Service: ${BLUE}${SERVICE_NAME}${NC} in region: ${BLUE}${REGION}${NC}"
echo -e "Security constraint: ${RED}Private IAP (No public unauthenticated access)${NC}"
...
# 8. Enable Identity-Aware Proxy (IAP) directly on Cloud Run service
echo -e "\n${YELLOW}Activating Google Cloud Identity-Aware Proxy (IAP) protection...${NC}"
...
echo -e "\n${GREEN}====================================================${NC}"
echo -e "${GREEN}🎉 Teacher Hub Deployment & IAP Activation Successful!${NC}"
echo -e "====================================================${NC}"
if [ -n "$SERVICE_URL" ]; then
    echo -e "Your OpenGoo! Secure Teacher Hub is now live at:"
    echo -e "${BLUE}${SERVICE_URL}${NC}"
    echo -e "\n${YELLOW}Security Check:${NC}"
    echo -e "- IAP is currently enabled on this service."
    echo -e "- Remember to whitelist authorized teachers' accounts in IAM by assigning them the role:"
    echo -e "  ${GREEN}IAP-secured Web App User${NC} (roles/iap.httpsResourceAccessor)"
else
